<?php
  include 'connect.php';
  class Location extends connect
  {
   ...
   public function save()
   {
      if($this->db_found==true)
      {
        ...
        echo "<div style='color: red; font-weight: bold';>⚠️ ID already exists.</div>";
           ...
           {
              $sql="insert into location values('$_POST[t1]','$_POST[t2]')";
  ...
  public function delete()
   {
       if($this->db_found==true)
       {
            $sql="delete from Location where id='$_POST[t1]'";
            mysqli_query($this->db_found,$sql);
    ...
    public function Update()
   {
       if($this->db_found==true)
       {
          $sql="Update  location set nm='$_POST[t2]',address='$_POST[t3]' where id='$_POST[t1]'";
          mysqli_query($this->db_found,$sql);
  ...
  public function Psearch()
  {
     if($this->db_found==true)
     {
          $s="select*from location where id='$_POST[t1]'";
          $r=mysqli_query($this->db_found,$s);
          echo "<table border=2 bgcolor=pink>
          <tr>
          <th> Id</th>
  ...
  public function Special_search()
  {
      if ($this->db_found == true) 
      {
          $value = $_POST["t1"];
          $field = $_POST["s"];
          $s = "SELECT * FROM location WHERE $field = '$value'";
          $r = mysqli_query($this->db_found, $s);
          echo "<table border=2 bgcolor=pink>
          <tr>
          <th> Id</th>
...
echo"<form name=f method=post action=Location.php>
<div class=container>
<h1>LOCATION</h1>
      <center>
         <table border=1>
       ...
       <tr>
             <th>Address</th>
             <th><input type=text name=t3 value=$ob->address></th>
        </tr>
       <tr>
